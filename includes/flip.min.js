...
//////////////////////////////////  ДОБАВЛЕНО  /////////////////////////////////////////////////////////////////////////////////////////////////
let timerevent = document.getElementById('timerevent');
let formattimerevent = timerevent.getAttribute('attformat');
let bgimage = timerevent.getAttribute('attfon');
let monthAlltimerevent = ['января','февраля','марта','апреля','мая','июня','июля','августа','сентября','октября','ноября','декабря'];
let todayflip = new Date();
let usersetdatef = new Date();
let userdateflip;
let fonurlhost = document.location.protocol;
let fonurlsite = document.location.host;
//let fonpath = fonurlhost + '//' + fonurlsite + '/timeuntil/img/';
let fonpath=''
if(timerevent.hasAttribute('attdate')){
    userdateflip = timerevent.getAttribute('attdate');
}
let userdatefliparr = userdateflip.split('-'); // гг мм дд
usersetdatef.setDate(userdatefliparr[2]);
usersetdatef.setMonth(userdatefliparr[1]-1);
usersetdatef.setFullYear(userdatefliparr[0]);

let locale = {
            YEAR_PLURAL: 'Лет',
            YEAR_SINGULAR: 'Лет',
            MONTH_PLURAL: 'Месяцев',
            MONTH_SINGULAR: 'Месяцев',
            WEEK_PLURAL: 'Недель',
            WEEK_SINGULAR: 'Недель',
            DAY_PLURAL: 'Дней',
            DAY_SINGULAR: 'Дней',
            HOUR_PLURAL: 'Часов',
            HOUR_SINGULAR: 'Часов',
            ...
            MILLISECOND_SINGULAR: 'Милисекунд'
        };

if(timerevent.hasAttribute('cycleday')){
    let when = timerevent.getAttribute('cycleday');
    
    timerevent.innerHTML = '<div class="flipfon"><div class="tick-label tick-text htimehere"></div>'
                         + '<div class="tick tomiddle" data-did-init="handleTickInit">'
                         + '<div data-repeat="true" data-layout="horizontal fit" data-transform="preset(' + formattimerevent + ') -> delay"><div class="columnticks"></div>'
                         + '<div class="tick-group">'
                         + '<div data-key="value" data-repeat="true" data-transform="pad(00) -> split -> delay">'
                         + '<span data-view="flip"></span></div>'
                         + '<span data-key="label" data-view="text" class="tick-label"></span> </div> </div> </div>'
                         + '<div class="fliptable"></div></div>';
    
    let bgevent = document.querySelector('.flipfon');
    //bgevent.style.backgroundImage = "url('./img/" + bgimage + "')";
    //bgevent.style.backgroundImage = "url('" + fonpath + bgimage + "')";
    let htimehere = document.querySelector('.htimehere');
    
    let timetext = parseInt(userdatefliparr[2]) + ' ' + monthAlltimerevent[parseInt(userdatefliparr[1]-1)] + ' ' + userdatefliparr[0];
    let formatfliparr = formattimerevent.split(',');
    ...
    function handleTickInit(tick) {
        ...
        let attname = timerevent.getAttribute('attname');
        
        let htime = 'Прошло с ' + attname;
        htimehere.innerHTML = htime;
        
        Tick.count.schedule(when, {format: formatfliparr}).onupdate = function(value) {
            tick.value = value;
            if(timerevent.hasAttribute('column')){
                let tickshere = document.querySelector('.tickshere');
                let tgroup = document.querySelectorAll('.tick-group');
                tickshere.style.display = 'grid';
                tickshere.style.justifyContent = 'center';
                bgevent.style.paddingTop = '10%';
                tickshere.style.margin = '50px auto';
            }
        }
        
        if(timerevent.hasAttribute('notitle')){
            htimehere.style.display = 'none';
        }
    }
}

else{
    timerevent.innerHTML = '<div class="flipfon"><div class="tick-label tick-text htimehere"></div><div class="tick tomiddle"'
            + 'data-did-init="handleTickInit">'
            + '<div data-repeat="true"'
            + 'data-layout="horizontal center fit"'
            + 'data-transform="preset(' + formattimerevent + ') -> delay" class="tickshere">'
            + '<div class="tick-group">'
            + '<div data-key="value"'
            + 'data-repeat="true"'
    ...
    if(timerevent.hasAttribute('cycle')){
        if(todayflip > usersetdatef){
            usersetdatef.setFullYear(todayflip.getFullYear());
            if(todayflip > usersetdatef){
                usersetdatef.setFullYear(usersetdatef.getFullYear() +1);
            }
            userdatefliparr[0] = usersetdatef.getFullYear();
            userdateflip = userdatefliparr.join('-');
            timetext = parseInt(userdatefliparr[2]) + ' ' + monthAlltimerevent[parseInt(userdatefliparr[1]-1)] + ' ' + userdatefliparr[0];
        }
    }
    ...
    function handleTickInit(tick) {
        ...
        if(todayflip >= usersetdatef){
            htime = 'Прошло';
            htimehere.innerHTML = htime;
            Tick.count.up(userdateflip, {format: formatfliparr}).onupdate = function (value) {
                tick.value = value;
                if(timerevent.hasAttribute('column')){
                    let tickshere = document.querySelector('.tickshere');
                    let tgroup = document.querySelectorAll('.tick-group');
            ...
            htime = 'Осталось';
            htimehere.innerHTML = htime;
            let p = userdateflip + 'T00:00:00';
            let counter = Tick.count.down(p, {format: formatfliparr});
...
if(timerevent.hasAttribute('table')){
   let fliptable = document.querySelector('.fliptable');
   fliptable.innerHTML = '<div class="middle2">'+'<div id="washere2"></div>'
            +'<div class="clockandlabelt2"><span id="year_textt" class="dyeart itlabelt2">Лет </span><span id="yeart" class="dyeart numlt2"></span></div> '                                       
            + '<div class="clockandlabelt2"><span id="month_textt" class="dmontht itlabelt2">Месяцев </span><span id="montht" class="dmontht numlt"></span></div> '
            + '<div class="clockandlabelt2"><span id="week_textt" class="dweek itlabelt2">Недель </span><span id="weekt" class="dweekt numlt2"></span></div> '                                      
            + '<div class="clockandlabelt2"><span id="day_textt" class="ddayt itlabelt2">Дней </span><span id="dayt" class="ddayt numlt2"></span></div> '
            + '<div class="clockandlabelt2"><span id="hour_textt" class="dhourt itlabelt2">Часов </span><span id="hourt" class="dhourt numlt2"></span></div> '
            + '<div class="clockandlabelt2"><span id="min_textt" class="dmint itlabelt2">Минут </span><span id="mint" class="dmint numlt2"></span></div> '
            + '<div class="clockandlabelt2"><span id="sec_textt" class="dsec itlabelt2">Секунд </span><span id="sect" class="dsec numlt2"></span></div></div>';
        
        let todaytime = new Date();
        let userdate = new Date();
        userdate.setFullYear(userdatefliparr[0]);
        userdate.setMonth(userdatefliparr[1]-1);
        userdate.setDate(userdatefliparr[2]);

        let fgt = document.getElementsByClassName('flipfon')[0];
        fgt.style.paddingTop = '10px';
    
        let timertest = setTimeout(function testtime(){
            gettable(todaytime, userdate);
            timertest = setTimeout(testtime, 1000); 
        },1000)
        ...
        function gettable(datemin, datemax){    
            ...
            todaytime = new Date();
            yeart.innerHTML = todaytime.getFullYear() - userdate.getFullYear();  yeart.innerHTML=notminus(yeart.innerHTML);
            let dt = (datemax.getTime() - datemin.getTime());
                ...
                dayt.innerHTML = Math.floor(dt / 1000 / 60 / 60 / 24) - 1;      
